...
class NpcManager():
    def __init__(self, map):
        ...
        npcs = self.map.game.game_data_db.execute("SELECT npc.id, npc.nom, x_coord, y_coord FROM npc JOIN maps ON npc.map_id = maps.id WHERE maps.file = ?", (map.current_map,)).fetchall()
